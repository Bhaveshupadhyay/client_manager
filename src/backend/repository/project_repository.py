...
from backend.services.redis_service import cached, get_redis_key
...
class ProjectRepository:
    ...
    async def _invalidate_project_cache(self, project_id: str):
        try:
            cache_key = get_redis_key("project_requirements", [project_id])
            await get_redis_client().delete(cache_key)
        except Exception as e:
            logger.warning(f"Failed to invalidate cache for project {project_id}: {e}")
    ...
    async def update_project(self, project_item: ProjectItem):
        ...
        try:
            json_data = project_item.model_dump()
            json_data['updated_at'] = datetime.now(timezone.utc).isoformat(timespec='seconds').replace("+00:00", "Z")

            await self.cosmos_db_container.replace_item(
                item=item_id,
                body=json_data
            )
            await self._invalidate_project_cache(item_id)
    ...
    async def create_project(self, project_item: ProjectItem):
        try:
            ...
            await self._invalidate_project_cache(project_item.project_id)

        except exceptions.CosmosResourceExistsError as e:
            raise RuntimeError("Project was created by another process. Please try updating again.") from e

        except exceptions.CosmosHttpResponseError as e:
            logger.error(f"Database error creating project {project_item.project_id}: {e}")
